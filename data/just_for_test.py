...
class Window(QMainWindow):
    ...
    def helpWindow(self):
        msg = QMessageBox()
        msg.setText(
            "Changing the seed will change the bonds to a different random system. You can type the configurations listed above into the 'config' box to see one of the lowest-energy state solutions for this particular system. Click below for a more detailed explanation.")
        msg.setDetailedText(
    ...
    def drawBonds_spins(self, qp, coordList):
        for i in range(self.N):
            for j in range(i + 1, self.N):
                if int(self.Jij[i][j] * (2 * self.configuration[-i - 1] - 1) * (
                        2 * self.configuration[-j - 1] - 1)) == -1:  # bond broken
    ...
    def drawStrings(self, qp):
        ...
        for string in strings:
            ...
            qp.drawPoint(self.coordList[string[0]][0] + (self.spacing) / 2,
                         self.coordList[string[0]][1] + (self.spacing) / 2)
            qp.drawPoint(self.coordList[string[-1]][0] + (self.spacing) / 2,
                         self.coordList[string[-1]][1] + (self.spacing) / 2)
        return
    ...
    def cantFind(self):
        ...
        msg.setDetailedText(
            "The algorithm assumes that the minimum posible distance between the frustrated plaquettes will result in a ground state. With periodic boundary conditions, this is not always the case. Therefore, some configurations are not yet working with this algorithm.")
        x = msg.exec()
        if self.xchange:
        ...
        for p2 in self.f_plaq[index + 1:]:
            ...
            if p1 > p2:
                ...
                pair_list.append((p1, p2,
                                  op_dist))  # Pair_list does not have true distance between pairs, the true distance is subtracted from the max possible distance to help with node matching later
    return pair_list
...
def viable_edges(self):
    ...
    for plaq2 in f_plaq:  # We start looping through each frustrated plaquette
        ...
        if first:
            for pair in p_pairs:  # Remember p_pairs is the list of pairs in the ground state we found above
                if pair[0][1] == plaq2 or pair[0][0] == plaq2:
                    ...
                    G2.remove_edge(
                        *pair[0])  # Remove the edge in our graph that has already been added to edge_lst above
                    break
        ...
        while ground_energy == True:
            ...
            if len(matching) != len(
                    f_plaq) / 2:  # This would happen if we have taken out all edges for a particular plaquette
                ground_energy = False
...
def plaq_groups(self):
    ...
    while running:
        for group_index, p_edges in enumerate(edges[current_plaq:]):
            ...
            if group_index + current_plaq == len(
                    edges) - 1:  # if we get to the last group of edges for the last possible plaquette without having a full ground state, we need to use a different combination of edges
                try_new = False
                for_loop = False
                for e_ind, edge in enumerate(group[
                                             ::-1]):  # going through the ground state group backwards to see if other edge choices will work
                    loop_count += 1
                    ...
                    if try_new == True:
                        ...
                        for e_index, edge2 in enumerate(edges[plaq_ind]):
                            if edge2 == edge:
                                if e_index == len(edges[
                                                      plaq_ind]) - 1 and plaq_ind == 0:  # end of program, we've reached the last entry of the first plaquette
                                    running = False
                                    new = True
                                    try_new = True
                                    break
                                elif e_index == len(edges[
                                                        plaq_ind]) - 1:  # Move to the previous plaquette list to find a viable edge
                                    break
                                else:
            ...
            for pair in p_edges[
                        p_ind:]:  # Here we are looping through each single edge from the edges associated with the current plaquette
                ...
                # break
                    ...
                    used_plaquettes.append(
                        pair[0][1])  # maybe add the first element of pair if this is the first plaquette we visit
                    ...
                    if len(group) == len(f_plaq) // 2:  # Group is full
                        ...
                        if length == ground_dist:
                            all_groups.append(
                                group)  # once we have filled a group, we know it will be good so we can add it to all_groups
                            # print('group added: ', group)

                        last_pair = group[
                            -2]  # This is the pair that we remove and replace before cycling through other options
                        ...
                        while found == False:
                            ...
                            for index, pairing in enumerate(edges[ind]):
                                if pairing == last_pair and index == len(edges[
                                                                             ind]) - 1:  # This happens if we are at the last pair of a particular plaquette
                                    if len(
                                            group) == 0:  # This happens if we have gotten through the last edge of the first plaquette, function is done
                                        running = False
                                        found = True
                                        break
                                    last_pair = group[
                                        -1]  # Take off the last pair and go to that plaquette to see if there are further pairs to use
...
def add_all_strings(self):
    ...
    for group in groups:  # loops through each group of plaquette pairs
        ...
        for pairing in group:  # each plaquette pair in the group
            paths = nx.all_shortest_paths(G, pairing[0][0],
                                          pairing[0][1])  # finds all possible paths between two points
            paths_list = []
            for path in paths:
